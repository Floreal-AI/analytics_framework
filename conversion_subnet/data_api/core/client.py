...
class VoiceFormAPIClient:
    ...
    def _parse_training_response(self, data: Dict[str, Any]) -> TrainingData:
        ...
        required = ['train_features', 'train_targets', 'features_list', 'round_number', 'refresh_date']
    ...
    def _parse_test_response(self, data: Dict[str, Any]) -> TestData:
        ...
        required = ['test_features', 'features_list', 'submissionDeadline']
        missing = [field for field in required if field not in data]
        if missing:
            raise DataValidationError(f"Missing required fields: {missing}")
    ...
    async def fetch_training_data(
        ...
        Args:
            ...
            save: Whether to save data as CSV to /data/train_data.csv (default: True)
        ...
        if save:
            self._save_training_data_csv(training_data)
        
        return training_data
    
    def _save_training_data_csv(self, training_data: TrainingData) -> None:
        """
        ...
        try:
            ...
            df = pd.DataFrame(training_data.features, columns=training_data.feature_names)
            df['target'] = training_data.targets
            
            # Save to CSV (overwrite existing)
            csv_path = data_dir / "train_data.csv"
    ...
    async def fetch_test_data(
        ...
        Args:
            limit: Number of records to fetch
            offset: Pagination offset
            save: Whether to save data as CSV to /data/test_data.csv (default: False)
        ...
        # Save as CSV if requested
        if save:
            self._save_test_data_csv(test_data)
        
        return test_data
    
    def _save_test_data_csv(self, test_data: TestData) -> None:
        """
        ...
        try:
            ...
            # Create DataFrame with features only (no targets for test data)
            df = pd.DataFrame(test_data.features, columns=test_data.feature_names)
    ...
    async def fetch_both(
        ...
        train_task = self.fetch_training_data(train_limit, train_offset, round_number, save_training)
        test_task = self.fetch_test_data(test_limit, test_offset, save_test)
